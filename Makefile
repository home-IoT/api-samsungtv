...
swagger-gen:
	mkdir -p gen
	swagger generate server -f $(SERVER_SWAGGER_FILE) -t gen -A $(PACKAGE_NAME)
...
go-build: 
	go build -ldflags="-X $(PACKAGE_PATH).GitRevision=$(shell git rev-parse HEAD) -X $(PACKAGE_PATH).BuildVersion=$(VERSION) -X $(JUPITER_PACKAGE).BuildTime=$(DATE)" -i -o ./bin/$(TARGET) gen/cmd/$(PACKAGE_NAME)-server/main.go
